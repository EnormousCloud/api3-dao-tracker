...
lazy_static! {
    pub static ref UP: IntGauge = register_int_gauge!(opts!("up", "Server is running")).unwrap();
}

lazy_static! {
    pub static ref CHAIN_ID_GAUGE: IntGauge =
        register_int_gauge!(opts!("sync_chain_id", "Chain ID that is being syncing",)).unwrap();
    pub static ref WATCHING: IntGauge =
        register_int_gauge!(opts!("sync_watching", "Whether the chain is being watched",)).unwrap();
    pub static ref BLOCK_START_GAUGE: IntGauge = register_int_gauge!(opts!(
        "sync_block_start",
        "Number of block that is being synced",
    ))
    .unwrap();
    pub static ref BLOCK_END_GAUGE: IntGauge = register_int_gauge!(opts!(
        "sync_block_end",
        "Number of block that is being synced",
    ))
    .unwrap();
...
lazy_static! {
    pub static ref SINCE_LAST_UPDATE: IntGauge = register_int_gauge!(opts!(
        "sync_since_last_update",
        "Seconds since the last state update",
    ))
    .unwrap();
    pub static ref NUM_ADDRESSES: IntGauge =
        register_int_gauge!(opts!("dao_addresses", "Number of addresses in DAO",)).unwrap();
    pub static ref NUM_VOTINGS: IntGauge =
        register_int_gauge!(opts!("dao_votings", "Number of votings in DAO",)).unwrap();
    pub static ref EPOCHS: IntGauge =
        register_int_gauge!(opts!("dao_epochs", "Number of epochs",)).unwrap();
    pub static ref EPOCH_INDEX: IntGauge =
        register_int_gauge!(opts!("dao_epoch_index", "Index of the current epoch",)).unwrap();
    pub static ref APR: Gauge =
        register_gauge!(opts!("dao_apr", "Index of the current epoch",)).unwrap();
    pub static ref LAST_BLOCK: IntGauge =
        register_int_gauge!(opts!("dao_last_block", "Index of the last block",)).unwrap();
    pub static ref GENESIS_APR: Gauge =
        register_gauge!(opts!("dao_genesis_apr", "Genesis APR",)).unwrap();
    ...
    pub static ref STAKE_TARGET: Gauge =
        register_gauge!(opts!("dao_stake_target", "Stake Target",)).unwrap();
    pub static ref UNSTAKE_WAIT_PERIOD: IntGauge =
        register_int_gauge!(opts!("dao_unstake_wait_period", "Unstake Wait Period",)).unwrap();
    pub static ref REWARD_PERIOD: IntGauge =
        register_int_gauge!(opts!("dao_reward_vesting_period", "Reward Vesting Period",)).unwrap();
    pub static ref CIRCULATING_SUPPLY: Gauge =
        register_gauge!(opts!("dao_circulating_supply", "API3 Circulating Supply",)).unwrap();
    pub static ref TOTAL_SUPPLY: Gauge =
        register_gauge!(opts!("dao_total_supply", "API3 Total Supply",)).unwrap();
    pub static ref LOCKED_GOV: Gauge =
        register_gauge!(opts!("dao_locked_by_governance", "Locked By Governance",)).unwrap();
    pub static ref LOCKED_REWARDS: Gauge =
        register_gauge!(opts!("dao_locked_rewards", "Locked Rewards",)).unwrap();
    pub static ref LOCKED_VESTINGS: Gauge =
        register_gauge!(opts!("dao_locked_vestings", "Locked Vestings",)).unwrap();
    pub static ref TIME_LOCKED: Gauge =
        register_gauge!(opts!("dao_time_locked", "Time Locked",)).unwrap();
    pub static ref TOTAL_LOCKED: Gauge =
        register_gauge!(opts!("dao_total_locked", "Total Locked",)).unwrap();
    pub static ref SINCE_LAST_VOTINGS: IntGauge = register_int_gauge!(opts!(
        "dao_since_last_votings_read",
        "Seconds since the last votings read",
    ))
    .unwrap();
    pub static ref SINCE_LAST_ENS: IntGauge = register_int_gauge!(opts!(
        "dao_since_last_ens_read",
        "Seconds since the last ENS read",
    ))
    .unwrap();
    pub static ref SINCE_LAST_TREASURIES: IntGauge = register_int_gauge!(opts!(
        "dao_since_last_treasuries_read",
        "Seconds since the last treasuries read",
    ))
    .unwrap();
    pub static ref SINCE_LAST_CIRCULATION: IntGauge = register_int_gauge!(opts!(
        "dao_since_last_circulation_read",
