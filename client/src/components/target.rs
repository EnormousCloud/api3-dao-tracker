...
pub fn staking_note<T>(apr: f64, stake_target: U256, total_staked: U256) -> Node<T> {
    ...
    let (color, txt) = if is_min {
        ("var(--color-warning)", "DAO staking target is reached, and APR is at its minimum of 2.5%")
    } else if is_max {
        ("var(--color-accent)", "DAO staking target is not reached, and APR is at its maximum of 75%")
    } else if reached {
